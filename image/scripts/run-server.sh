...
UDP2RAW_CONF="${UDP2RAW_CONF:-/config/udp2raw/server.conf}"
UDPSPEEDER_CONF="${UDPSPEEDER_CONF:-/config/udpspeeder/server.conf}"
...
echo "[SecretWireguard] Starting udp2raw (server)"
/usr/local/bin/udp2raw --conf-file "${UDP2RAW_CONF}" &
UDP2RAW_PID=$!

echo "[SecretWireguard] Starting UDPspeeder (server)"
